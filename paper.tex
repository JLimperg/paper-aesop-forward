...
A \textit{shared variable} of $r$ is an input premise $x_i$ such that there are at least two input premise types $T_j$, $T_k$ in which $x_i$ occurs.
...
\paragraph{Example 1.}
\xcom{This is quite verbose, maybe I should use a more systematic approach.}
...
This updates the map $\eta_y(1, y \mapsto 302)$ with $\{h_1\}$.
An update in slot 1 also marks the begining of a new partial match and we update de match map $\mu_y$ acordingly.
Note that after the creation of a match of level 1, we look at the hypotheses in slot 2 and check if they can extend the match to level 2. However, the instantiation of the current hypotheses in slot 2 are inconsistent with $y \mapsto 302$, so nothing can be extended.
...
In this particuliar case, since $h_3$ is in slot $2$ we look for $s(2) = \{x,y\}$.
Then,
\[
...
In~\cite{TODO}, it is shown to be good solution for the problem of finding terms (here the types of hypotheses) that might unify with other terms (here the types of the input premises).
...
One of the more costly operation that we need to run multiple times is the unification \xcom{defeq?} between the hypotheses and the types of the input premises associated to rules.
