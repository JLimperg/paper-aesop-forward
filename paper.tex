...
\]
At this point, $H$ and $M$ are in the state shown in Diagram~\eqref{eq:example1:state1}.
...
There are no compatible matches at level~1 so we are done with $h₂$, yielding the final rule state in Diagram~\eqref{eq:example1:state2}.
    ...
    \begin{minipage}{0.55\textwidth}
      ...
      \begin{equation}\label{eq:example1:state2}
        ...
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}, outer sep=auto, level distance=4em, scale=0.5]
              \node at (-1, 0) {1}
              child {node[xshift=-3] {$c$}
                child {node {$\{[h₁]\}$}}}
              child {node[xshift=1] {$b$}
                child {node {$\{[h₂]\}$}}};
        \end{tikzpicture}
      \end{equation}
      \end{center}
...
We then add $h_2 : A~a~b$, which initially generates a match $m = [h₂]$ that is added to $M$, yielding the rule state in Diagram~\eqref{eq:example2:state1}:
  \begin{align}\label{eq:example2:state1}
    H_x : 
...
This yields the rule state in Diagram~\eqref{eq:example2:state2}.
...
\begin{align}\label{eq:example2:state2}
  H_x : 
  \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)},outer sep=auto, level distance=4em, scale=0.50]
    \node at (0, 0) {2}
    child {node {$a$}
