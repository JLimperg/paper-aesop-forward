...
We add hypotheses $h₁ : B~a~a$, $h₂ : A~a~b$ and $h₃ : B~a~b$ to the rule state of $r₂$, starting with $h₁$:
...
We now add $h_2 : A~a~b$, which initially generates a match $m = [h₂]$ that is added to $M$:
