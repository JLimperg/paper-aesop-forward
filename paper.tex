...
Hence, if $s$ is the forward state corresponding to context $Γ$ and $\addHyp(s, h : T) = (s', M)$, then $s'$ is the forward state corresponding to $Γ,\, h : T$.
...
The set of matches compatible with $h₂$ is supposedly $\bigcap_{x \in \vars(2)} M_{x}(i-1, σ(x)) = ∅$, but the match $\{1 ↦ h₁\}$ can be extended with $h₂$.
...
Since this is the first slot, we start a new partial match $\{1 ↦ h₁\}$, which Algorithm~\ref{alg:addmatch} inserts into the set $M_{y}(1, 302)$.
...
For slot 1, $\sub₁(h₂)(y) = 301$, so we add the match $\{1 ↦ h₂\}$ to $M_{y}(1, 301)$.
