...
In many proof assistants, simple \enquote*{white-box} automation tactics based on tableau-style tree search play a substantial role in practice: Isabelle's (TODO ref) and Rocq's (TODO ref) auto, ACL2's (TODO ref) waterfall, various PVS (TODO ref) tactics, Lean's (TODO ref) Aesop (TODO ref), etc.
...
When Aesop selects a forward rule $r : \All{(x₁ : A₁) \dots (xₘ : Aₘ)}{B}$ as the highest-priority rule currently applicable to a goal $Γ ⊢ T$, we first unify the last premise $Aₘ$ with each hypothesis $h : C$ such that $C$ and $Aₘ$ likely unify (as determined by a discrimination tree (TODO ref) index).
...
If the last premise is present, however, we perform a large number of unifications between premises and hypotheses (though some of these could perhaps be avoided heuristically (TODO ref fast subsumption)).
  ...
  \Procedure{$\mathrm{addHyp}(h : T)$}{
    \eIf{$i = 1$}{
      ...
      \For{$m$ \KwIn $\mathit{ms}$}{
        $\mathit{completeMs} \gets \mathit{completeMs} ∪ \mathrm{addMatch}(m ++ [h])$
      }
...
To account for this fact, we partition the set of premises of each rule into maximal subsets of premises connected by shared variables, using a construction from (TODO ref Aesop Sec. 4.2).
...
Since this is the first slot, we start a new partial match $[h₁]$, which Algorithm~\ref{alg:addmatch} inserts into the set $M_{y}(1, 302)$.
This match cannot be extended since the hypothesis index $H$ does not contain a hypothesis for slot 2.

For slot 2, unification yields the substitution $σ ≔ \{y ↦ 301, z ↦ 302\}$, so we insert $h₁$ into $H_{y}(2, 301)$.
No matches at level 1 can be extended by $h₁$ since
\[
  \bigcap_{x \in \vars(2)} M_{x}(1, σ(x)) = M_{y}(1, 301) = \emptyset
\]
...
\begin{figure}
    ...
    \begin{subcaptiongroup}
      ...
      \begin{minipage}{0.5\textwidth}
        \begin{tikzpicture}[outer sep=auto, level distance=4em, scale=0.50]
          ...
          \node {$H_y\colon$};
          \node at (1.5, 0) {2}
            child {node[xshift=-2] {$301$}
              child {node {$\{h₁\}$}}}
            child {node[xshift=2] {$300$}
              child {node {$\{h₂\}$}}};
          \node at (5, 0) {$M_y\colon$};
          \node at (6.5, 0) {1}
            child {node[xshift=-2] {$302$}
              child {node {$\{[h₁]\}$}}}
            child {node[xshift=2] {$301$}
              child {node {$\{[h₂]\}$}}};
        \end{tikzpicture}
  ...
  \begin{subcaptiongroup}
    ...
    \end{minipage}
  \end{subcaptiongroup}
  \caption{Rule states encountered during Examples~1 and~2}
\end{figure}

Now we add $h₂ : 300 ≤ 301$, which again unifies with both slots.
For slot 1, $\sub₁(h₂)(y) = 301$, so we add the match $[h₂]$ to $M_{y}(1, 301)$.
We then check for hypotheses in slot 2 that also instantiate $y$ with $301$, and indeed we find $M_{y}(2, 301) = \{h₁\}$.
Hence we return the complete match $[h₂, h₁]$, which induces a new hypothesis $h₃ : 300 ≤ 302$ to be added later.

For slot 2, we add $h₂$ to $H_{y}(2, 300)$.
...
Additionally, forward rules are currently little used by Aesop's main client, the Mathlib mathematics library (TODO ref).
We therefore only evaluate our technique against the previous, naive Aesop implementation on several synthetic benchmarks that showcase the performance characteristics of incremental forward reasoning.
    ...
    \begin{tikzpicture}[scale=0.75]
      \begin{axis}[
        ...
        \addplot[
          ...
          coordinates {
            (1, 7.663472) (2, 9.354750) (4, 15.768194) (8, 104.921791) (16, 1882.683111) % (32, 1175.888750) 
          };
        ...
        \addplot[
          ...
          coordinates {
            (1, 10.308541) (2, 10.704638) (4, 16.274305) (8, 38.800833) (16, 179.697681) % (32, 45220.919667) 
          };
...
An important detail is that the scales are logarithmic
...
In this case, we sketch the behaviour of the stateless algorithm:
It pick a first hypothesis, $h_{i,j} : P_{i,j}(a)$.
If $j < 6$, the index returns nothing and it selects another hypothesis.
For $h_{i,6} : P_{i,6}(a)$, the rule $r_i$ is selected, and it then needs to try and fill its remaining five slot with the context's $6n$ hypotheses which it tries all.
...
A SATCHMO-style prover may benefit from our incremental approach if the set of implications is large, though in a setting where unification is cheap it is not clear whether avoiding repeated unifications outweighs the cost of maintaining the forward state.
