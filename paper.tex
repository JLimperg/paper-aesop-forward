...
Second, we observe that the variable indices are only queried for hypotheses in a slot $i > 1$ when a match is added to slot $i - 1$.
Hence we delay the insertion of hypotheses at slot $i > 1$ until there is at least one match in slot $i - 1$.%
\footnote{This latter optimisation was suggested by Massin Guerdi.}
