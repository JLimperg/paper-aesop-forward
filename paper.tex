...
This helps, for example, with the definitional rule for involutive functions: if $f : α → α$ is involutive, then for any $a : α$ we have $f (f a) = a$.
When used as a forward rule, the premise $a$ matches any non-propositional hypothesis, but this is only relevant if there is, in fact, an involutive function is the context.
