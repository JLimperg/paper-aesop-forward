...
\xcom{I would remove either 1 or 2 from the graph (they have the same depth and then remove this explanation. I can then either modify the test accordingly or just move the explanation to the test.)}
The reason we see similar times for the naive algorithm at $1$ and $2$ is because for $1$, it tries to build the theorem using $P_n(a + 1)$ and immediately fails because $P_{n - 1}(a)$ is incompatible.
For $2$, the same thing happens but the instantiations are swapped.
For the next depths, the stateless algorithm gets progressively slower the further the incompatible hypothesis is located.
...
%Precompilation is by default turned off for Mathlib4 as it amplifies its already long compilation time.
%Users working on projects that uses a lot of forward reasoning can however chose to pay this compilation time price once and then benefit from fast forward reasoning.
...
\paragraph{SATCHMO.}
The SATCHMO first-order theorem prover (TODO ref) uses a search procedure that treats a given set of implications as forward rules and uses a single non-forward rule to perform case analysis on disjunctive hypotheses.
...
Some knowledge-based reasoning systems, e.g.\ Algernon (TODO ref) and EYE (TODO ref), support combined forward and backward reasoning.
