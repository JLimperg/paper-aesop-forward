...
In the last example, the incremental algorithm performs well partly because it can reuse much of its work.
We now consider a test with almost no potential for reuse.

Given $n ≥ 1$, we use $n$ forward rules of the form $\All{x}{P_{1}~x → \dots → P_{6}~x}$, where $P_{1}, \dots, P_{6}$ are arbitrary predicates that are distinct from each other and from the predicates of all other rules.
The initial goal contains one hypothesis of type $P_{i}~a$ for each predicate $P_{i}$ that appears in a rule, where $a$ is again either 0 or 100.
...
The is because the naive algorithm fails to take much advantage of the rules' independence:
whenever a hypothesis is added (and thus a new subgoal generated), it re-runs all the rules it had already run on the previous subgoal, realising only after the fact that the rules were already applied.
This yields a complexity of $O(n^{3})$ for the naive algorithm versus $O(n)$ for the incremental algorithm.
...
Our final benchmark concerns situations where a rule matches almost, but not quite.
We use a single rule $r : \All{x}{P_{1}~x → \dots → P_{n}~x → A}$, where the predicates $P_{i}$ are all distinct and $A$ is an arbitrary proposition.
\jcom{What is $n$?}
Given a \emph{depth} $k ≤ n$, the initial goal contains hypotheses of types $P_{i}~a$ for $1 ≤ i ≤ n$ and $i ≠ n - k$ as well as one hypothesis $P_{n-k}~(a + 1)$ (with $a = 0$ or $a = 100$ as before).
In other words, the hypotheses match all premises with compatible substitutions, except the $k$-th premise from the back.
Since the naive algorithm processes the premises back-to-front, $k$ corresponds to the number of premises that it matches before it becomes clear that the rule cannot be applied.

This setup is challenging for the incremental approach because (a) no rule is ever applied, so incrementality does not help; and (b) the incremental algorithm expends a non-negligible amount of effort for each of the $n$ hypothesis, whereas the naive algorithm can, at least for small $k$, decide fairly quickly that the rule is not applicable.
And indeed, Fig.~\ref{fig:erase-noprecomp} shows that the naive approach scales with $k$ while the incremental one does not, but the constant factors are so high that the incremental approach remains slower for $k$ up to 5.

Fig.~\ref{fig:erase-precomp}, however, shows a different picture, with the incremental approach leading in most cases.
The figure shows the results of running the same depth benchmark with \emph{precompilation} enabled.
This is a feature of Lean that compiles tactics ahead of time (whereas they are normally interpreted).
